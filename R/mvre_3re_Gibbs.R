#' @export
mvreprobit_3re_Gibbs <- function(Y, x_covs, j_ind, i_ind, jt_ind, max_steps, cor_step_size = 0.01){
  K <- ncol(Y)
  rcd_num <- nrow(Y)
  ind_num <- max(i_ind)
  grp_num <- max(j_ind)
  ...
  for(iter in 1:max_steps){
    ...
    # cat('sample U done |')
    # sample V
    V_all <- sample_X_all1_3re(V_all, Sigma_e_inv, Sigma_v_inv, it_len,
                                rowsum(Y_star - Xbeta - U_all[i_ind,] - W_all[jt_ind,], j_ind, reorder = T))
    # cat('sample V done |')
    # sample W
    W_all <- sample_X_all1_3re(W_all, Sigma_e_inv, Sigma_w_inv, i_len,
                                rowsum(Y_star - Xbeta - U_all[i_ind,] - V_all[j_ind,], jt_ind, reorder = T))
    # cat('sample W done')
    # update parameters
    params <- sample_params_3re(x_covs, XtX, Y_star, U_all, V_all, W_all, coeffs,
...
#' @export
mvreprobit_3re_Gibbs_addon <- function(Y, x_covs, j_ind, i_ind, jt_ind, max_steps, res, cor_step_size = 0.02){
  K <- ncol(Y)
  rcd_num <- nrow(Y)
  ind_num <- max(i_ind)
  grp_num <- max(j_ind)
  ...
  for(iter in 1:max_steps){
    ...
    cat('sample W done')
    # update parameters
    params <- sample_params_3re(x_covs, Y_star, U_all, V_all, W_all, coeffs,
                                 Sigma_e, Sigma_e_inv, Sigma_u, Sigma_v, Sigma_w, j_ind, i_ind, jt_ind, cor_step_size)
    coeffs <- params$coeffs
    Sigma_e <- params$Sigma_e
    Sigma_u <- params$Sigma_u
...
#' @export
res_summary_3re_Gibbs <- function(mvreprobit_res, xcov, burnin, chains2=NULL){
  if(!is.null(chains2)){
    for(i in 1:5)
